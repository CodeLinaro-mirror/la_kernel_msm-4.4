 ...
 *
 */
int qtee_shmbridge_init(struct device *dev, bool is_hyp_suppported)
{
	int ret = 0;
	uint32_t *ns_vm_ids;
...
//			"qcom,support-hypervisor");
	...
	if (!default_bridge.vaddr)
		return -ENOMEM;
	default_bridge.paddr = dma_map_single(dev,
				default_bridge.vaddr, default_bridge.size,
				DMA_TO_DEVICE);
	if (dma_mapping_error(dev, default_bridge.paddr)) {
		pr_err("dma_map_single() failed\n");
		ret = -ENOMEM;
		goto exit_freebuf;
	}
	default_bridge.dev = dev;
...
exit_unmap:
	dma_unmap_single(dev, default_bridge.paddr, default_bridge.size,
			DMA_TO_DEVICE);
exit_freebuf:
...
EXPORT_SYMBOL(qtee_shmbridge_init);
...
int qtee_shmbridge_deinit(struct device *dev)
{
	qtee_shmbridge_deregister(default_bridge.handle);
	gen_pool_destroy(default_bridge.genpool);
	dma_unmap_single(dev, default_bridge.paddr, default_bridge.size,
...
EXPORT_SYMBOL(qtee_shmbridge_deinit);
