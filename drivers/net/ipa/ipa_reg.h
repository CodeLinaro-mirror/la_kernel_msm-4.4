...
/* The next five fields are present for IPA v4.7+ */
...
static inline u32 arbitration_lock_disable_encoded(enum ipa_version version,
						   u32 mask)
{
	ipa_assert(NULL, version >= IPA_VERSION_4_0);
...
/* Note: IPA v3.0 uses a different set of BCR values (but bit 0 is the same) */
...
static inline u32 ipa_reg_bcr_val(enum ipa_version version)
{
	...
	val = BCR_CMDQ_L_LACK_ONE_ENTRY_FMASK;
	if (version == IPA_VERSION_3_0 || version == IPA_VERSION_3_1)
		return val;

	val |= BCR_SUSPEND_L2_IRQ_FMASK;
	val |= BCR_HOLB_DROP_L2_IRQ_FMASK;
	val |= BCR_DUAL_TX_FMASK;
	if (version == IPA_VERSION_4_0 || version == IPA_VERSION_4_1)
		return val;

	/* version == IPA_VERSION_3_5 || version == IPA_VERSION_3_5_1 */
	val |= BCR_TX_NOT_USING_BRESP_FMASK;
...
#define IPA_PIPES_FMASK				GENMASK(3, 0)
#define IPA_CONS_PIPES_FMASK			GENMASK(12, 8)
#define IPA_PROD_PIPES_FMASK			GENMASK(20, 16)
#define IPA_PROD_LOWEST_FMASK			GENMASK(27, 24)

/* The next register is present for IPA v3.5+ */
static inline u32 ipa_reg_idle_indication_cfg_offset(enum ipa_version version)
{
	ipa_assert(NULL, version >= IPA_VERSION_3_5);
...
static inline u32 ipa_resource_group_src_count(enum ipa_version version)
{
	...
	case IPA_VERSION_3_1:	/* XXX Not sure for this version */
		return 6;
...
/* Not all of the following are present (depends on the count, above) */
...
/* The next register is not present for IPA v4.2 (it has no hashing support) */
...
static inline u32 ipa_reg_irq_stts_ee_n_offset(enum ipa_version version, u32 ee)
{
	u32 offset = 0x1000 * ee;

	return offset + version < IPA_VERSION_4_7 ? 0x00003008 : 0x00004008;
}
...
static inline u32 ipa_reg_irq_en_ee_n_offset(enum ipa_version version, u32 ee)
{
	...
	return offset + version < IPA_VERSION_4_7 ? 0x0000300c : 0x0000400c;
}
...
static inline u32 ipa_reg_irq_clr_ee_n_offset(enum ipa_version version, u32 ee)
{
	...
	return offset + version < IPA_VERSION_4_7 ? 0x00003010 : 0x00004010;
}
...
static inline u32 ipa_reg_irq_uc_ee_n_offset(enum ipa_version version, u32 ee)
{
	...
	return offset + version < IPA_VERSION_4_7 ? 0x0000301c : 0x0000401c;
}
...
{
	ipa_assert(NULL, version != IPA_VERSION_3_0);
